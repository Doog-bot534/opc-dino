...
```
[新项目] Postwave — 出海 indie 的多平台 launcher，海外 API 直发 + 中国 Electron 容器
```

备选标题（如果第一条被 mod 限流）：
```
做了一个独立开发者的多平台发布工具，聊聊架构选型 — Postwave
```
...
Phase 0 — landing + waitlist 阶段。这条帖子也是用 Postwave 的内容生成（人工 + AI 辅助）方式发的，相当于 dogfood 第 0 个版本。
